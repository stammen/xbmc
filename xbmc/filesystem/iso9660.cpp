...
//******************************************************************************************************************
struct iso_dirtree *iso9660::FindFolderW(wchar_t *Folder)
{
  wchar_t *work;

  work = (wchar_t *)malloc(from_723(m_info.iso.logical_block_size) * sizeof(wchar_t));

  wchar_t *temp;
  struct iso_directories *lastpath = NULL;

  if (wcspbrk(Folder, L":"))
    wcscpy(work, wcspbrk(Folder, L":") + 1);
  else
    wcscpy(work, Folder);

  temp = work + 1;
  while (wcslen(temp) > 1 && wcspbrk(temp + 1, L"\\"))
    temp = wcspbrk(temp + 1, L"\\");

  if (wcslen(work) > 1 && work[wcslen(work) - 1] == '*')
  {
    work[wcslen(work) - 1] = 0;
  }
  if (wcslen(work) > 2)
    if (work[wcslen(work) - 1] == '\\')
      work[wcslen(work) - 1] = 0;
  ...
  {
    std::wstring strDllW;
    g_charsetConverter.utf8ToW(CSpecialProtocol::TranslatePath(lastpath->path), strDllW, false, false, false);
    if (!wcsicmp(strDllW.c_str(), work))
    {
      free(work);
      return lastpath->dir;
    }
...
//******************************************************************************************************************
HANDLE iso9660::FindFirstFileW9660(wchar_t *szLocalFolder, WIN32_FIND_DATA *wfdFile)
{
  if (m_info.ISO_HANDLE == 0) return (HANDLE)0;
  memset(wfdFile, 0, sizeof(WIN32_FIND_DATA));

  m_searchpointer = FindFolderW(szLocalFolder);
...
HANDLE iso9660::OpenFile(const char *filename)
{
  ...
  std::wstring strDllW;
  g_charsetConverter.utf8ToW(CSpecialProtocol::TranslatePath(filename), strDllW, false, false, false);

  pointer = strDllW.c_str();

  while (wcspbrk(pointer, L"\\/"))
    pointer = wcspbrk(pointer, L"\\/") + 1;

  wcscpy(work, pointer);
  pointer2 = work;

  while (wcspbrk(pointer2 + 1, L"\\"))
    pointer2 = wcspbrk(pointer2 + 1, L"\\");

  // Note const pointer2 is a pointer into non const work array
  wchar_t* temp = (wchar_t*)*(pointer2 + 1);
  temp = 0;

  intptr_t loop = (intptr_t)FindFirstFileW9660(&work[0], &fileinfo);
  while (loop > 0)
  {
    if (!wcsicmp(fileinfo.cFileName, pointer))
      loop = -1;
    else
